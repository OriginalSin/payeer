...
var maxTime = 0;
var delta = 10;
var ind = -1, indPrev = -1, lastBal = [100,0,100], priceSell = 0, priceSellPrev = 0, priceBuy = 0;
...
var getArr = function(){
	...
	for(var i=0; i<children.length-1; i++){
		var i4 = 4*i;
		var priceNew = Number(arr2[i4+1].replace(',','')); 
		var el = children[i];
		var elSell = el.classList.contains('trade-type-sell');
		if (elSell) {
			if (priceNew == priceSell) break;
			if (priceNew > priceSell) {
				if (priceSell > priceSellPrev) {
					ind = -1;
				}
				else {
					ind = 0;
			...
			priceSellPrev = priceSell; priceSell = priceNew;
		} 
		else priceBuy = priceNew;
		/*
